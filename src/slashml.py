...
from slashml.src.json_to_dot import edict

from enum import Enum

class SpeechToTextServiceProviders(Enum):
    ASSEMBLY = 'assembly'
    AWS = 'aws'
    WHISPER = 'whisper'
...
class SpeechToText:
    ...
    def status(self, job_id: str, service_provider: ServiceProvider, header=None):
        headers = self.HEADERS
        payload = {"service_provider": service_provider.value}

        response = requests.get(
            self.SLASHML_TRANSCRIPT_STATUS_URL(job_id), headers=headers, data=payload
        )
...
class Summarization:
    ...
    def status(self, job_id: str, service_provider: ServiceProvider, header=None):
        ...
        payload = {"service_provider": service_provider.value}
        response = requests.get(
            self.SLASHML_SUMMARIZE_STATUS_URL(job_id), headers=headers, data=payload
        )
